...
selection_out = reset_names_draft_picks.query(f'season >= {since_season}& (allpro>0 or probowls > 0)').groupby(['team',above_3rd_round],group_keys = True).count()[['allpro','probowls']]
...
for curr_repeat_list in repeat_list:
    ...
    temp_sum = 0
    for z in curr_repeat_list:
        temp_sum += selection_out[selection_out['team'] == z]['allpro_probowl'].values[0]
