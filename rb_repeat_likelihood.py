...
#%% load relevant years
relevant_years = np.arange(2010,2024).tolist()
...
len_data = len(top_20rbs[top_20rbs['season']<= max(relevant_years)-2])

#%% repeat 1 season fantasy finish

number_repeats = np.zeros(great_thresh)
for m in range(1,great_thresh+1):
    for p in np.arange(2010,max(relevant_years)-1).tolist():
...
number_repeats = np.zeros(great_thresh)
for m in range(1,great_thresh+1):
    for p in np.arange(2010,max(relevant_years)-2).tolist():
        curr_player_id = top_20rbs[(top_20rbs["Season_Finish"] == m) & (top_20rbs["season"] == p)]["player_id"].values[0]
...
curr_ax.set_xlabel('Season Fantasy Finish (Rank)')
curr_ax.set_title(f'Likelihood >8 games following Top {great_thresh} Finish',
                  y =1,pad = +1)
